...
describe('Login', () => {
    ...
    it("Can't login with empty email", () => {
        login.loginUser('', Cypress.env('password'), 'email').then((response) => {
    ...
    it("Can't login with empty password", () => {
        login.loginUser(Cypress.env('registeredEmail'), '', 'email').then((response) => {
            expect(response.status).to.eq(400);
    ...
    it("Can't login with invalid email", () => {
        login.loginUser('invalidemail', Cypress.env('password'), 'email').then((response) => {
            expect(response.status).to.eq(400);
    ...
    it("Can't login with invalid password", () => {
        login.loginUser(Cypress.env('registeredEmail'), 'invalidpassword', 'email').then((response) => {
            expect(response.status).to.eq(400);
    ...
    it("Can't login with unregistered email", () => {
        login.loginUser(Cypress.env('newEmail'), Cypress.env('password'), 'email').then((response) => {
            expect(response.status).to.eq(400);
    ...
    it("Can't login with password less than 8 characters", () => {
        login.loginUser(Cypress.env('registeredEmail'), '1234567', 'email').then((response) => {
            expect(response.status).to.eq(400);
