...
let userToken, vendorToken, driverToken, role, branchId, serviceId, offeringId, orderId, gigId;
...
describe('Create Bidding For Delivery API Testing', () => {
    ...
    describe('After Login', () => {
        ...
        describe("User is an approved Driver and tries to create Bidding to deliver the order at user's house", () => {
        ...
        describe("User tries to create a gig without being switched to driver role", () => {

            it('should throw error on trying to create a Gig', () => {
                role = 'user';
                if(selfDelivery === false){
                    createBidding(userToken, gigId, randomBidOption).then((response) => {
        ...
        describe("Vendor tries to create a gig", () => {

            it('should throw error on trying to create a Gig', () => {
                role = 'vendor';
                if(selfDelivery === false){
                    createBidding(vendorToken, gigId, randomBidOption).then((response) => {
                        expect(response.status).to.eq(403);
                        expect(response.body).to.have.property('message', `${commonError.forbidden} ${role} mode.`);
                    });
                }else{
    ...
    describe('Without Login', () => {
        ...
        it('should throw error on trying to create a Bid', () => {
            createBidding('', gigId, randomBidOption).then((response) => {
