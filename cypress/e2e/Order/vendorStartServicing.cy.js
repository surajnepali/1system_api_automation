...
let userToken, vendorToken, driverToken, branchId, serviceId, offeringId, gigId, orderId, selfPickup;
let role, gigType, randomBidOption, randomBidId;
...
describe('Vendor Start Servicing API Testing', () => {
  ...
  describe('After Login', () => {
    ...
    describe('User is an approved Vendor tries to start servicing', () => {
          ...
          it('Vendor should accept the order', () => {
            acceptOrderByVendor(orderId, vendorToken).then((response) => {
              ...
              expect(response.body).to.have.property(
                'message', `${orderSuccessMessages.successful}accepted`
              );
      ...
      describe('When user tries to re-start the servicing of the order', () => {
        
        it('should throw error on trying to re-start the servicing by the vendor', () => {
          vendorStartServicing(orderId, vendorToken).then((response) => {
      ...
      describe('Unappropriate vendor is trying to start servicing', () => {
        ...
        it('should throw erroe on trying to start servicing by any other vendor', () => {
          vendorStartServicing(orderId, vendorToken).then((response) => {
            expect(response.status).to.eq(400);
            expect(response.body).to.have.property(
              'message',
  ...
  describe('Without Login', () => {
    it('should throw error on trying to start servicing', () => {
      vendorStartServicing(orderId, '').then((response) => {
