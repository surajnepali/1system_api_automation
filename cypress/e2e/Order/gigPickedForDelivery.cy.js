...
import { orderAccessEmails, createOrderData } from "../../api/Order_APIs/order.data";
import { commonSuccessMessages, vendorSuccessMessages, orderSuccessMessages, driverSuccessMessages, userSuccessMessages } from "../../message/successfulMessage";
...
import { getAllGigs, getGigDetails, createBidding, pickGig } from "../../api/Driver_APIs/driver.api";
...
describe('Create Bidding For Delivery API Testing', () => {
  describe('GET BranchId, ServiceId, OfferingId', () => {
    before(() => {
      login(
        ...
        expect(response.body).to.have.property(
          'message',
          `${commonSuccessMessages.sucessfulLogin}`
        );
    ...
    it('should switch to vendor role', () => {
      role = 'vendor';
      cy.log('Vendor Token: ' + userToken);
      switchRole(role, userToken).then((response) => {
        expect(response.status).to.eq(200);
        expect(response.body).to.have.property(
          'message',
          `${commonSuccessMessages.switchedTo} ${role}`
        );
        expect(response.body.data).to.have.property('token');
  ...
  describe('After Login', () => {
    describe('User is an approved Driver', () => {
      describe("Tries to pick the gig to deliver at user's house", () => {
        ...
        it('should create order successfully', () => {
          ...
          createOrder(x, mainUserToken).then((response) => {
            ...
            expect(response.body).to.have.property(
              'message',
              `${orderSuccessMessages.successful}created`
            );
        ...
        it('Vendor should accept the order', () => {
          acceptOrderByVendor(orderId, vendorToken).then((response) => {
            ...
            expect(response.body).to.have.property(
              'message',
              `${orderSuccessMessages.successful}accepted`
            );
        ...
        it('should login with the driver email', () => {
          if (selfPickup === false) {
            ...
            cy.log(
              'Self Pickup is true, so no need to login with driver email'
            );
          }
        });
        ...
        it('should get all the gigs', () => {
          if (selfPickup === false) {
            getAllGigs(driverToken, pageOptions.PAGE, pageOptions.LIMIT).then(
              (response) => {
                expect(response.status).to.eq(200);
                expect(response.body).to.have.property(
                  'message',
        ...
        it('should get gig details', () => {
          if (selfPickup === false) {
            getGigDetails(driverToken, gigId).then((response) => {
              ...
              randomBidOption =
                gigBiddingOptions[
                  Math.floor(Math.random() * gigBiddingOptions.length)
                ];
        ...
        it('should view the list of biddings by the user', () => {
          if (selfPickup === false) {
            viewBiddings(
              ...
              expect(response.body).to.have.property(
                'message',
                `${userSuccessMessages.biddingRetrieved}`
              );
        ...
        it('should accept a bid successfully', () => {
          if (selfPickup === false) {
            acceptBid(orderId, randomBidId, gigType, mainUserToken).then(
              (response) => {
                cy.log(orderId);
                expect(response.status).to.eq(200);
                expect(response.body).to.have.property(
                  'message',
                  `${userSuccessMessages.bidAccepted}`
                );
              }
            );
          }
            ...
            getOrders(
              ...
              for (let i = 0; i < orders.length; i++) {
                if (orders[i].id === orderId) {
                  expect(orders[i]).to.have.property(
                    'status',
                    orderApiOptions.PICKEDUP
                  );
        ...
        it('should start servicing by the vendor', () => {
          vendorStartServicing(orderId, vendorToken).then((response) => {
            ...
            expect(response.body).to.have.property(
              'message',
              `${orderSuccessMessages.isNow} in servicing.`
            );
          });
        });
        ...
        it('should finish the servicing by the vendor', () => {
          vendorFinishServicing(orderId, vendorToken).then((response) => {
            ...
            expect(response.body).to.have.property(
              'message',
              `${orderSuccessMessages.isNow} ready..`
            );
          });
        });
        ...
        describe('Driver 1 makes a bid', () => {
          it('should login with the driver email', () => {
            if (selfDelivery === false) {
              ...
              cy.log(
                'Self Delivery is true, so no need to login with driver email'
              );
            }
          });
          ...
          it('should switch to driver role', () => {
            ...
            if (selfDelivery === false) {
              switchRole(role, userToken).then((response) => {
                ...
                expect(response.body.data).to.have.property('token');
                driverToken = response.body.data.token;
                const decodedToken = jwt.decode(driverToken);
                drivers.push({
                  driver_id: decodedToken.driver_id,
                  driver_token: driverToken,
                });

                cy.log('Driver Array: ' + JSON.stringify(drivers));
              });
            } else {
              cy.log(
                'Self Delivery is true, so no need to switch to driver role'
              );
            }
          });
          ...
          it('should get gig details', () => {
            if (selfDelivery === false) {
              getGigDetails(driverToken, gigId).then((response) => {
                ...
                expect(response.body.data).to.have.property('gig');
                expect(response.body.data.gig).to.be.an('object');
                expect(response.body.data.gig).to.have.property(
                  'gig_id',
                  gigId
                );
        ...
        describe('Driver 2 makes a bid', () => {
          it('should login with the driver email', () => {
            if (selfDelivery === false) {
              login(
                orderAccessEmails.approvedDriverEmail2,
                Cypress.env('password'),
                'email'
              ).then((response) => {
                expect(response.status).to.eq(200);
        ...
        describe('Driver picks the order from the Vendor', () => {
      ...
      describe('Driver tries to pick the gig again that is already accepted', () => {
        it('should throw error on trying to pick a bid', () => {
          if (selfDelivery === false) {
            const selectedDriver = drivers.find((driver) => driver.driver_id === driverId);
            if (selectedDriver) {
              pickGig(selectedDriver.driver_token, gigId).then((response) => {
            ...
            cy.log(
              'Self Delivery is true, so no need to accept the gig again that is already accepted'
            );
          }
        });
      });

      describe('Inappropriate driver tries to pick the gig', () => {
        it('should throw error on trying to pick a bid', () => {
          if (selfDelivery === false) {
            const selectedDriver = drivers.find((driver) => driver.driver_id !== driverId);
            if (selectedDriver) {
            ...
            cy.log(
              'Self Delivery is true, so no need to pick the gig'
            );
          }
        });
      });
    ...
    describe('Vendor tries to pick the gig', () => {
      it('should throw error on trying to pick a Gig', () => {
        role = 'vendor';
        if (selfDelivery === false) {
    ...
    describe('User tries to pick a gig', () => {
      it('should throw error on trying to pick a Gig', () => {
        role = 'user';
        if (selfDelivery === false) {
