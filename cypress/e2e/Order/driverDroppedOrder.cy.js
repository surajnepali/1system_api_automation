...
describe('Create Bidding For Delivery API Testing', () => {
    ...
    describe('After Login', () => {
        ...
        describe('User is an approved Driver', () => {
            ...
            describe("Tries to pick the gig to deliver at user's house", () => {
                ...
                it('should pick the gig', () => {
                    if(selfDelivery === false){
                        ...
                        driverToken = selectedDriver.driver_token;
                        if (selectedDriver) {
                        ...
                        if (selectedDriver) {
                            cy.fixture('person.jpg', 'binary')
                                ...
                                .then((blob) => {
                                    ...
                                    orderDroppedbyDriver(gigId, formData, selectedDriver.driver_token).then((response) => {
                                        expect(response.status).to.eq(200);
                                    })
                                })
                        }
            ...
            describe('Driver tries to drop the gig again that is already dropped', () => {
                it('should throw error on trying to drop a gig that is already dropped', () => {
                    if (selfDelivery === false) {
                        const selectedDriver = drivers.find((driver) => driver.driver_id === driverId);
                        if (selectedDriver) {
                            cy.fixture('person.jpg', 'binary')
            ...
            describe('Inappropriate driver tries to drop the gig', () => {
                it('should throw error on trying to drop the gig', () => {
                    if (selfDelivery === false) {
                        const selectedDriver = drivers.find((driver) => driver.driver_id !== driverId);
                        if (selectedDriver) {
                            cy.fixture('person.jpg', 'binary')
        ...
        describe('Vendor tries to drop the gig', () => {
            it('should throw error on trying to drop the gig', () => {
                role = 'vendor';
                if (selfDelivery === false) {
                    cy.fixture('person.jpg', 'binary')
                        ...
                        .then((blob) => {
                            ...
                            orderDroppedbyDriver(gigId, formData, vendorToken).then((response) => {
        ...
        describe('User tries to drop the gig', () => {
            it('should throw error on trying to drop the Gig', () => {
                role = 'user';
                if (selfDelivery === false) {
                    cy.fixture('person.jpg', 'binary')
                        ...
                        .then((blob) => {
                            ...
                            orderDroppedbyDriver(gigId, formData, userToken).then((response) => {
                                expect(response.status).to.eq(403);
                            })
        ...
        it('should throw error on trying to pick a Bid', () => {
            cy.fixture('person.jpg', 'binary')
                ...
                .then((blob) => {
                    ...
                    orderDroppedbyDriver(gigId, formData, '').then((response) => {
